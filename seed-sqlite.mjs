...
const dbUtils = {
  ...
  // Get a single row
  async getRow(text, params) {
    const database = await initDatabase();
    return await database.get(text, params);
  },

  // Get multiple rows
  async getRows(text, params) {
    const database = await initDatabase();
  ...
  async insert(text, params) {
    ...
    // For SQLite with custom IDs, we need to get the inserted row by the ID we provided
    const tableName = text.match(/INSERT INTO (\w+)/)?.[1];
    if (tableName && params && params[0]) {
      return await database.get(`SELECT * FROM ${tableName} WHERE id = ?`, [params[0]]);
    }
    return result;
  },
  ...
  try {
    ...
    await database.exec(`
      CREATE TABLE IF NOT EXISTS employees (
        ...
        work_location TEXT,
        is_ssnit_contributor INTEGER DEFAULT 1,
        is_tier3_contributor INTEGER DEFAULT 0,
        tier3_contribution_percentage REAL DEFAULT 5.0,
